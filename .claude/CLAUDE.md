...
- After each phase, run a **post-phase adversarial code review** over the phase's diff — **single reviewer by default** (prefer `mcp__codex__codex`, else one `adversarial-reviewer` subagent), escalating to a **dual** review only for risky phases (package-API / cross-cutting / security-sensitive). Apply the fixes, commit. Skip the review when the phase doesn't require in-depth review (docs-only, trivial config, mechanical renames). Full triggers: `.claude/PARALLEL.md` §7.
