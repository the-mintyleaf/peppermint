...
**Unit-targeted routing (REQ §6.7):** since no dispatcher position exists, a unit routing target resolves directly to the unit head, then nearest ancestor head — the "active dispatcher position" branch of REQ §6.7 is documented as *not applicable* until `organization` introduces such a concept.
