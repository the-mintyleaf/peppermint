...
└── modules/
    ...
    └── admin/
        ...
        └── dashboard/        # Not-Contained (reporting) — IS the /admin home; 8 independent read-only sections, no CRUD
            ├── dashboard.{types,api,queryKeys,hooks,labels,utils,chartConfig}.ts   # one fetch fn + one useQuery per section, never combined
            ├── pages/{AdminHome,DashboardOverview}.tsx   # AdminHome = /admin entry (RequireAuth + role branch)
            └── components/{SuperadminLanding,DashboardHeaderControls,SummaryStrip,TodayWorklists,PipelineCounts,Blockers,Workload,Conversion,Outcomes,ActivityFeed}.tsx  # charts via @peppermint/ui/charts (DonutStat, CategoryBarChart, StackedBarChart, Gauge)
```
