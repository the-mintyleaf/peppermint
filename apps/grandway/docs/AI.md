...
└── modules/
    ...
    └── admin/
        ...
        │   └── pages/
        │       ├── list/                 # DataTableShell; inline ApplicantStatusSwitch in Status column; row actions: OpenDocumentButton (admin-only, open/create doc workspace) + menu (View/Edit). The same ApplicantStatusSwitch is the detail header's status control
        │       ├── new/, edit/           # FormShell-wrapped ApplicantForm
        │       └── detail/               # 2-col ProfileLayout: sticky sidebar grouped Identity/Contact/Address/Record | underline tabs Passport & Family / Journeys / Documents / Files / Alerts / History — every panel opens with ProfilePanelHeader
        ...
        └── dashboard/        # Not-Contained (reporting) — IS the /admin home; 8 independent read-only sections behind an Overview + 6 detail tabs, no CRUD
            ├── dashboard.{types,api,queryKeys,hooks,labels,utils,chartConfig}.ts   # one fetch fn + one useQuery per section, never combined
            ├── dashboard.tabs.ts   # the 7 tab keys + per-tab question/caveat; active tab is URL state (?tab=)
            ├── pages/{AdminHome,DashboardOverview}.tsx   # AdminHome = /admin entry (RequireAuth + role branch); DashboardOverview = hero + DashboardTabs
            └── components/{SuperadminLanding,DashboardHero,DashboardHeaderControls,DashboardTabs,OverviewPanel,StatTile,StatTiles,CountryCards,ChecklistItemRowView,TodayWorklists,PipelineCounts,Blockers,Workload,Conversion,Outcomes,ActivityFeed}.tsx  # charts via @peppermint/ui/charts (DonutStat, CategoryBarChart, StackedBarChart, Gauge)
```
...
| Dashboard                | `modules/admin/dashboard`                     | `/admin` (it IS the home)                                                                                                                            | Not-Contained (reporting) — 8 independent read-only sections (summary/today/pipeline/blockers/workload/conversion/outcomes/activity), each its own request/query; a slow or failing section must never block the others. Presented as an **Overview signal board + 6 detail tabs** (`?tab=`, URL state like the filters) with `keepMounted={false}`, so a section fetches when its tab is opened; Overview reads tiles (all 11 `summary` figures) → country cards → charts → the overdue/due-soon checklist queues, each card carrying one "Open <tab>" button. The country cards are N independent `summary` calls, one per country — `country` is a FILTER, not a group-by, so there is no backend breakdown to read; the visible count is the request count and is bounded on purpose (8, +8 per "Show more", no "show all"). Access is decided at the `/admin` home (`AdminHome` role branch — admin/lead_manager only); superadmin gets the `SuperadminLanding` fallback and is 403'd on every endpoint anyway. There is no separate "Dashboard" rail entry — the always-shown "Home" entry leads here (the dashboard for admin/lead_manager, `SuperadminLanding` for superadmin). Header controls (module header right slot) expose only `fiscal_year`+`country` — the shared filter set's 4 other fields (`journey_stage`/`offer_status`/`document_status`/`checklist_status`) are validated-and-ignored by the backend and deliberately have no UI control. `workload.is_scoped_to_caller` changes only the section's heading ("My workload" vs "Workload by owner") — the underlying lists arrive already scoped server-side; do not re-filter them client-side. Visualizations are Mantine Charts via `@peppermint/ui/charts` (DonutChart/BarChart breakdowns + magnitudes, semicircle-donut gauges); conversion's 4 rates and outcomes' 2 windowing rules must never be combined into one figure. Base path `/api/v1/dashboard/` |
...
- `RequireLeadAccess` (`components/RequireLeadAccess`) — `admin`/`lead_manager`; gates
  Leads, Applicants, Applicant Journeys, Institutions (Catalogue), Offers, Clients,
  Checklists (including template _reads_ — only the 4 template-write routes are
  additionally gated inline to `authorityType === "admin"`), Notifications, and Dashboard
  (reused as-is — the name is a holdover from Leads being first, but the rule is identical
  across these domains' backends). Deliberately the mirror image of `RequireStaff`: a
  `superadmin` never reaches any of these modules (each backend 403s that tier on every
  endpoint), while a `lead_manager` — who cannot use `RequireStaff`-gated areas — can.
  Note the read/write split within some of them: Institutions, Clients, and Checklists
  (templates) share reads with `lead_manager` but gate **writes** to
  `authorityType === "admin"` (Offers has no split — full rights for both).
  Any component surfacing a summary of this data outside the module itself (e.g. the
  dashboard home's role branch in `AdminHome`) must gate on the same rule —
  `authorityType === "admin" || authorityType === "lead_manager"`, not `isAdmin` alone
  (`isAdmin` is also true for `superadmin`).
- `RequireDocumentAccess` (`components/RequireDocumentAccess`) — exact `admin` only
  (denies both `lead_manager` and `superadmin`). Gates the Documents module and
  Uploaded Files' review queue (`/admin/files/review`). Do **not** reach for this to gate
  a screen where `lead_manager` has read access (e.g. checklist templates) — it blocks
  reads that backend explicitly grants; use `RequireLeadAccess` plus inline admin-only
  gating on the write controls instead.
