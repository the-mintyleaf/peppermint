...
- `RequireLeadAccess` (`components/RequireLeadAccess`) — `admin`/`lead_manager`; gates
  Leads, Applicants, Applicant Journeys, Institutions (Catalogue), Offers, and Clients
  ...
  Note the read/write split within some of them: Institutions and Clients share reads with
  `lead_manager` but gate **writes** to `authorityType === "admin"` (Offers has no split —
  full rights for both).
