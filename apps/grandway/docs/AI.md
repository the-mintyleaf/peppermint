...
└── modules/
    ...
    └── admin/
        ...
        ├── notifications/    # ContainedModule — read/act-only inbox (no create endpoint exists anywhere)
        ...
        │   └── pages/centre/  # NotificationCentre — inbox grouped client-side by due_bucket
...
- **The sidebar notification bell.** `layouts/admin/Admin.tsx` calls
  ...
  into `buildAdminConfig`'s `unreadNotificationCount`; `config/nav/admin-nav.ts` renders it
