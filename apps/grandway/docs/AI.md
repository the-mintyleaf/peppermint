...
│       └── dashboard/page.tsx                            # → ModuleDashboardOverview
...
└── modules/
    ...
    └── admin/
        ...
        └── dashboard/        # Not-Contained (reporting) — 8 independent read-only sections, no CRUD anywhere
            ├── dashboard.{types,api,queryKeys,hooks,labels,utils}.ts   # one fetch fn + one useQuery per section, never combined
            └── components/{SummaryStrip,TodayWorklists,PipelineCounts,Blockers,Workload,Conversion,Outcomes,ActivityFeed,DashboardFilterBar}.tsx
```
...
| Dashboard                | `modules/admin/dashboard`                     | `/admin/dashboard`                                                                                                                                   | Not-Contained (reporting) — 8 independent read-only sections (summary/today/pipeline/blockers/workload/conversion/outcomes/activity), each its own request/query; a slow or failing section must never block the others. `RequireLeadAccess`; superadmin gets 403 on every endpoint (hidden from nav, guard excludes it). Filter bar exposes only `fiscal_year`+`country` — the shared filter set's 4 other fields (`journey_stage`/`offer_status`/`document_status`/`checklist_status`) are validated-and-ignored by the backend and deliberately have no UI control. `workload.is_scoped_to_caller` changes only the section's heading ("My workload" vs "Workload by owner") — the underlying lists arrive already scoped server-side; do not re-filter them client-side. No charts — plain numbers/tables only (conversion's 4 rates and outcomes' 2 windowing rules must never be combined into one figure). Base path `/api/v1/dashboard/` |
...
- **The sidebar notification bell.** `layouts/admin/Admin.tsx` calls
  ...
  grandway-specific.
- **Dashboard drill-throughs.** Every row on `/admin/dashboard` links to the owning
