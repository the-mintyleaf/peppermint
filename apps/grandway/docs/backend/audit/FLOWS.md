...
1. **Audit log** — open the log, optionally apply filters →
   ...
   - _Failure — `PERMISSION_DENIED`:_ the caller is not an administrator — hide the audit UI.
2. **Event detail** — open one row →
   `GET /api/v1/audit/events/<id>/` (`audit.event.read`)
   - **Requires state:** a valid administrator token.
   - **Side effects:** none.
   - _Failure — `AUDIT_EVENT_NOT_FOUND`:_ the event id is unknown — show not-found.
...
1. **Authentication activity** — filter to one account's auth events →
   `GET /api/v1/audit/events/?app=authenticate&actor_id=<uuid>` (`audit.event.list`), or by `entity_type=authenticate.user&entity_id=<uuid>` for events _about_ the account.
   - **Requires state:** a valid administrator token.
   - **Side effects:** none.
