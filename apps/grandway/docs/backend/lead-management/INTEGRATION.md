...
**Version:** 1.0.0
**Status:** Active
**Synced:** 2026-07-23 (from `.backend/backend/leads/docs/{API,DATA_CONTRACT,INTEGRATION,SECURITY}.md`, `.backend/backend/leads/{serializers,constants,views,urls,exceptions}.py`, and `.backend/iterations/20260723_1635_applicant_journeys_applicants_core_leads.md` — the backend's own `docs/API.md` Change History line still reads "conversion deferred to Phase 4," but `LeadConvertView`/`urls.py` and the iteration digest confirm it shipped this session; this doc follows the code + iteration digest over that stale line)
...
| 1.1.0   | 2026-07-23 | Convert documented (18th endpoint) — shipped same-day; `applicants`/`applicant_journeys` flipped from future to real dependencies |
...
- **Name:** Leads — tracks people who have shown interest in the consultancy
  ...
  last followed up, and whether they were lost or converted into an applicant.
...
| `applicants`         | service call  | Convert calls `applicants.services.create_applicant` to create the applicant record.                                              | Convert would fail — no applicant to link the lead to.                                |
| `applicant_journeys` | service call  | Convert calls `applicant_journeys.services.create_journey` to create the seed journey.                                            | Convert would fail — no journey to seed from `study_interest`.                        |
...
- `converted_at`/`converted_at_bs`/`converted_by`/`converted_applicant_id`/`converted_journey_id` populate together at conversion and are **never cleared, not even by reopen**. `converted_applicant_id`/`converted_journey_id` are bare id strings for navigating into `applicants`/`applicant_journeys`.
...
| `/api/v1/leads/<lead_id>/convert/`   | POST   | `leads.lead.convert`         | **Admin only**                        | Any active stage qualifies; empty body; creates applicant + seed journey |
...
| `LEADS_LEAD_ALREADY_CONVERTED`   | 409  | "This lead has already been converted into an applicant."                                                                                         | Read `converted_applicant_id` off the lead and navigate there rather than retrying |
| `LEADS_CONVERSION_NOT_READY`     | 409  | "A lost or converted lead must be reopened before it can be converted."                                                                           | Prompt to reopen first                                                             |

## 9. Gaps

- **Direct applicant creation** (Admin creating an applicant without a lead) belongs to the `applicants` app — not this module.
