...
| `StatTile`             | One tile in three registers: tone-tinted header band (entity icon — the glyph that entity wears in the nav rail — + label) · the figure at display size · footer (`caption` + state pill: "Breached"/"At risk"/"Queued", or "Clear" at zero; volumes carry no pill). Tone drives the tint: volume=teal, critical=red, warning=orange, routine=violet. `—` on error, never `0`. `onActivate` makes it a button (and the only tiles that lift on hover) |
