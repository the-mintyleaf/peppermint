...
- **`keepMounted={false}`** — only the open tab's queries fire, so no tab pays for
  ...
  - `summary` is the **one exception** — `DashboardHero` sits above the tabs and
    ...
    are. `NeedsAttention` reuses the same cached query.
  - `overview` additionally mounts `pipeline`, `today`, `blockers`, `conversion`
    (4 more) — the four sections its glance cards actually show.
