  ...
  return (
    <form onSubmit={form.onSubmit((values) => onSubmit(values as never))}>
      <Stack gap="md" p="md">
        ...
        <FormSection
          title="Transactions"
          description="First row is the opening balance. Insert Interest / Tax rows anywhere — each is computed from the rows above it and re-syncs automatically."
          actions={
            <Group gap="xs">
              <Button
                size="xs"
        ...
        >
          ...
          <Paper withBorder p="xs" bg="var(--mantine-color-gray-0)">
            <Group justify="space-between" gap="xl">
              ...
              <Text size="xs" c="dimmed">
                Closing Balance:{" "}
                <b>{money(computed.statement_balance_total)}</b>
              </Text>
            </Group>
          </Paper>
        </FormSection>
