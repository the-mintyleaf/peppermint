...
import { Slider, NumberInput, Text, Stack, Group, useDebouncedCallback } from "@zetsel/ui";
import { useDocumentEditor } from "../../context";
import type { DocumentConfigBarProps, BankContent } from "../../documents.types";

const inputStyles = {
  label: { fontSize: "var(--mantine-font-size-xs)" },
  input: { fontSize: "var(--mantine-font-size-xs)", minHeight: 24, height: 24, padding: "0 6px" },
};

const ATTR = "data-bank-padding-active";

export function BankConfigBar({ document: doc, onUpdate, disabled }: DocumentConfigBarProps) {
  ...
  const [paddingTop, setPaddingTop] = useState<number>((hp.height as number) ?? 1);
  const [paddingBottom, setPaddingBottom] = useState<number>((hp.paddingBottom as number) ?? 0.5);
  ...
  const debouncedUpdate = useDebouncedCallback((top: number, bottom: number) => {
    const c = contentRef.current as BankContent;
    onUpdate({
      ...c,
      headerProps: { ...(c.headerProps ?? {}), height: top, paddingBottom: bottom },
    });
  }, 300);
  ...
  const handleTopSliderChange = (val: number) => {
    ...
    markUnsavedChanges();
    const c = contentRef.current as BankContent;
    onUpdate({ ...c, headerProps: { ...(c.headerProps ?? {}), height: val, paddingBottom } });
  };
  ...
  const handleBottomSliderChange = (val: number) => {
    ...
    markUnsavedChanges();
    const c = contentRef.current as BankContent;
    onUpdate({ ...c, headerProps: { ...(c.headerProps ?? {}), height: paddingTop, paddingBottom: val } });
  };
  ...
  return (
    <Stack gap="md" p="xs">
      <Stack gap={4}>
        <Text fz="xs" fw={500} c="dimmed">Top Padding</Text>
      ...
      <Stack gap={4}>
        <Text fz="xs" fw={500} c="dimmed">Bottom Padding</Text>
        <Group gap="xs" align="center">
          <Slider
            value={paddingBottom}
