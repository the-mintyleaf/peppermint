...
import { Badge, Button, ModuleHeader, notifications, Paper, TextInput } from "@peppermint/ui";
...
import { useOrgTreeStore, type OrgFlowNode, type OrgFlowEdge } from "./OrganizationTree.store";
import type {
  OrgNodeData,
  DepartmentData,
  PersonData,
...
function getEdgeStyleForRelationship(relType: string | undefined, isPath: boolean, isDim: boolean) {
  if (isPath) {
    return {
      style: { strokeWidth: 3, stroke: "var(--mantine-color-orange-5, #f97316)" },
      markerEnd: { type: MarkerType.ArrowClosed, color: "var(--mantine-color-orange-5, #f97316)" },
    };
  }
  if (isDim) {
    return {
...
function autoArrangeNodes(nodes: OrgFlowNode[], edges: OrgFlowEdge[], layoutMode: "compact" | "expanded" = "expanded"): OrgFlowNode[] {
  ...
  if (roots.length === 0) {
    return nodes.map((n, i) => ({
      ...n,
      position: { x: (i % 4) * (NODE_W + BASE_H_GAP), y: Math.floor(i / 4) * (NODE_H + BASE_V_GAP) },
    }));
  }
  ...
  function subtreeWidth(id: string, parentChildCount = 1): number {
    ...
    return Math.max(NODE_W, childWidths.reduce((acc, w) => acc + w + hGap, -hGap));
  }
  ...
  function layout(id: string, x: number, y: number, parentChildCount = 1) {
    ...
    const totalWidth = kids.map((k) => subtreeWidth(k, kids.length)).reduce((a, w) => a + w + hGap, -hGap);
...
function nodeMatchesSearch(type: string, data: OrgNodeData, q: string): boolean {
  switch (type) {
    case "person":
      return (data as PersonData).fullName.toLowerCase().includes(q) ||
...
function OrganizationTreeInner() {
  const {
    selectedNodeId, drawerOpen, activeDepartmentId, searchQuery, nodeModal, saved,
    closeDrawer, openAddModal, openEditModal, closeModal, setSearchQuery,
    pushHistory, undo, redo, canUndo, canRedo,
    markSaved, markDirty, contextMenu, openContextMenu, closeContextMenu,
    setActiveDepartment, setHighlightedNodeId,
    expandedNodeIds, expandedGroupIds, focusedBranchId, viewMode,
    expandNode: storeExpandNode, collapseNode, collapseAll,
    setFocusedBranch, setViewMode, setExpandedNodeIds, syncEdgeCache,
    activeFilters, searchMatchIds, setSearchMatchIds,
    selectNode,
  } = useOrgTreeStore();
  ...
  const graphMaps = useMemo(() => ({
    ...
    nodeTypeMap: new Map<string, string | undefined>(nodes.map((n) => [n.id, n.type])),
    nodeDataMap: new Map<string, Record<string, unknown>>(nodes.map((n) => [n.id, n.data as Record<string, unknown>])),
  ...
  const pathNodeIds = useMemo(
    () => (selectedNodeId ? new Set(computePathFromRoot(selectedNodeId, edges, graphMaps.parentMap)) : new Set<string>()),
    [selectedNodeId, edges, graphMaps]
  );

  // ── Dimmed nodes ──────────────────────────────────────────────────────────
  const dimmedNodeIds = useMemo(
    () => new Set(computeDimmedNodeIds(focusedBranchId, [...visibleIdsWithGroups], edges, graphMaps.childrenOf, graphMaps.parentMap)),
  ...
  const descendantStatsMap = useMemo(() => {
    ...
    for (const id of visibleIdsWithGroups) {
      map.set(id, computeDescendantStats(id, nodes, edges, childrenOf, nodeTypeMap));
    }
    return map;
  ...
  const healthIssuesMap = useMemo(() => {
    ...
    for (const id of visibleIdsWithGroups) {
      const issues = computeNodeHealth(id, nodes, edges, childrenOf, nodeTypeMap, nodeDataMap, descendantStatsMap.get(id));
  ...
  // ── Search match IDs ──────────────────────────────────────────────────────
  const activeSearchMatchIds = useMemo(() => new Set(searchMatchIds), [searchMatchIds]);
      ...
      for (const filter of activeFilters as FilterKey[]) {
        switch (filter) {
          ...
          case "leadership_only":
            if (type !== "person") { passes = false; break; }
            // eslint-disable-next-line no-case-declarations
            const pRole = (data as PersonData).role;
            if (!["head", "manager", "minister", "secretary", "joint_secretary"].includes(pRole ?? "")) passes = false;
            break;
          case "health_issues_only":
            if (!healthIssuesMap.has(id)) passes = false;
            break;
          case "empty_depts_only":
            if (!healthIssuesMap.get(id)?.includes("empty_dept")) passes = false;
            break;
          case "no_head_only":
            if (!healthIssuesMap.get(id)?.includes("missing_head")) passes = false;
            break;
          case "active_only":
            if ((data as { status?: string }).status !== "active") passes = false;
            break;
          case "inactive_only":
            if ((data as { status?: string }).status !== "inactive") passes = false;
            break;
        }
        if (!passes) break;
      }
  ...
  const enrichedVisibleNodes = useMemo(() => {
    ...
    return nodes
      ...
      .filter((n) => !q || nodeMatchesSearch(n.type ?? "", n.data, q) || n.type === "group")
      .map((n) => ({
        ...n,
        data: {
          ...n.data,
          ...
          _directChildCounts: computeDirectChildCounts(n.id, nodes, edges, childrenOf, nodeTypeMap),
          _descendantStats: descendantStatsMap.get(n.id),
          _healthIssues: healthIssuesMap.get(n.id) ?? [],
        },
      }));
  }, [
    nodes, edges, filteredVisibleIds, expandedNodeIds, pathNodeIds,
    dimmedNodeIds, activeSearchMatchIds, descendantStatsMap, healthIssuesMap, searchQuery, graphMaps,
  ]);

  // ── Enrich visible edges ──────────────────────────────────────────────────
  const enrichedVisibleEdges = useMemo(() => {
    return edges
      .filter((e) => filteredVisibleIds.has(e.source) && filteredVisibleIds.has(e.target))
      .map((e) => {
  ...
  useEffect(() => {
    ...
    const visEdges = currentEdges.filter(
      (e) => filteredVisibleIds.has(e.source) && filteredVisibleIds.has(e.target)
    );
  ...
  const breadcrumbPath = useMemo(() => {
    ...
    const pathIds = computePathFromRoot(focusedBranchId, edges, graphMaps.parentMap);
    return pathIds.map((id) => {
      const node = graphMaps.nodesMap.get(id);
      const label = node?.data.nodeType === "person"
        ? (node.data as PersonData).fullName
        : node?.data.nodeType === "group"
        ? (node.data as GroupData).name
        : (node?.data as OrgOfficeData | DepartmentData | undefined)?.name ?? id;
      return { id, label };
    });
  ...
  const analyticsStats = useMemo(() => {
    let totalOrgs = 0, totalDepts = 0, totalPeople = 0, missingHeads = 0, emptyDepts = 0, inactivePeople = 0;
    for (const n of nodes) {
    ...
    return { totalOrgs, totalDepts, totalPeople, missingHeads, emptyDepts, inactivePeople };
  }, [nodes, healthIssuesMap]);

  const selectedNode = graphMaps.nodesMap.get(selectedNodeId ?? "") ?? null;
  const activeDepartment = activeDepartmentId ? graphMaps.nodesMap.get(activeDepartmentId) : null;
  const activeDepartmentName = activeDepartment?.data.nodeType === "department"
  ...
  const onNodeClick: NodeMouseHandler<OrgFlowNode> = useCallback((_event, node) => {
    selectNode(node.id);
  }, [selectNode]);

  // Double-click = enter focus mode
  const onNodeDoubleClick: NodeMouseHandler<OrgFlowNode> = useCallback((_event, node) => {
    setFocusedBranch(node.id);
  }, [setFocusedBranch]);

  const onNodeContextMenu: NodeMouseHandler<OrgFlowNode> = useCallback((event, node) => {
  ...
  }, [closeDrawer, closeContextMenu, setActiveDepartment, setHighlightedNodeId]);
  ...
  const handleAddNode = useCallback(
    ...
    [setNodes, setEdges, edges, pushHistory, markDirty, nodeModal.pendingParentId, storeExpandNode]
  );
  ...
  const handleEditNode = useCallback(
    (data: OrgNodeData) => {
      ...
      setNodes((ns) => {
        const updated = ns.map((n) => n.id === nodeModal.editingNodeId ? { ...n, data } : n);
        pushHistory(updated, edges);
        return updated;
      });
      markDirty();
  ...
  const handleRequestDelete = useCallback((nodeId: string) => {
    ...
    if (!node) return;
    const { childrenOf, nodeTypeMap } = graphMaps;
    const stats = computeDescendantStats(nodeId, nodes, edges, childrenOf, nodeTypeMap);
    const nodeName =
      node.data.nodeType === "person" ? (node.data as PersonData).fullName
      : node.data.nodeType === "group" ? (node.data as GroupData).name
  ...
  const handleAutoArrange = useCallback((layoutMode: "compact" | "expanded" = "expanded") => {
    const visibleNodeList = nodes.filter((n) => filteredVisibleIds.has(n.id));
    ...
    setNodes((ns) => {
      const updated = ns.map((n) => (posMap[n.id] ? { ...n, position: posMap[n.id] } : n));
      pushHistory(updated, edges);
      return updated;
    });
    setTimeout(() => fitView({ duration: 400, padding: 0.15 }), 50);
  ...
  const handleSave = useCallback(() => {
    notifications.show({ title: "Saved", message: "Organization structure saved successfully.", color: "teal" });
    markSaved();
  }, [markSaved]);
  ...
  const editingNode = nodeModal.editingNodeId ? graphMaps.nodesMap.get(nodeModal.editingNodeId) : undefined;

  // Context menu — type-aware actions
  const getContextMenuActions = useCallback((nodeId: string) => {
    ...
    const actions = [
      ...
      !isPerson ? { label: "Expand one level", action: () => storeExpandNode(nodeId) } : null,
      !isPerson ? { label: "Collapse branch", action: () => collapseNode(nodeId) } : null,
      { label: "Edit", action: () => openEditModal(nodeId) },
      (isOrg || isDept) ? { label: "Add department", action: () => openAddModal("department", nodeId, (node?.data as OrgOfficeData | DepartmentData).name) } : null,
      (isOrg || isDept) ? { label: "Add person", action: () => openAddModal("person", nodeId, (node?.data as OrgOfficeData | DepartmentData).name) } : null,
      { label: "Delete", action: () => handleRequestDelete(nodeId), danger: true },
    ].filter(Boolean) as Array<{ label: string; action: () => void; danger?: boolean }>;
    return actions;
  }, [graphMaps, selectNode, setFocusedBranch, storeExpandNode, collapseNode, openEditModal, openAddModal, handleRequestDelete]);
  ...
  const headPersonForOrg = useMemo(() => {
    ...
    for (const childId of childIds) {
      ...
      if (child?.type === "person") {
        ...
        if (["head", "manager", "minister", "secretary"].includes(pData.role ?? "")) {
  ...
  const selectedDescendantStats = selectedNodeId ? descendantStatsMap.get(selectedNodeId) : undefined;
  const selectedHealthIssues = selectedNodeId ? (healthIssuesMap.get(selectedNodeId) ?? []) : [];

  // Person drawer stats
  const personDirectReports = selectedNode?.type === "person"
    ? computeDirectChildCounts(selectedNode.id, nodes, edges, graphMaps.childrenOf, graphMaps.nodeTypeMap).personCount
    : 0;
  ...
  return (
    <Paper p={0} withBorder radius="md" style={{ height: "calc(100vh - 16px)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <ModuleHeader
        breadcrumbItems={[
        ...
        right={
          <div style={{ display: "flex", alignItems: "center", gap: 8, paddingRight: 12 }}>
            <TextInput
              ...
              leftSection={<MagnifyingGlassIcon size={13} aria-label="Search" />}
              defaultValue={searchQuery}
              onChange={(e) => handleSearchChange(e.target.value)}
              style={{ width: 200 }}
            />
            {activeFilters.length > 0 && (
              <Badge size="sm" color="indigo" variant="light">{activeFilters.length} filter{activeFilters.length > 1 ? "s" : ""}</Badge>
            )}
            {enrichedVisibleNodes.length > 0 && (
              <Badge size="sm" color="gray" variant="light">{enrichedVisibleNodes.length} visible</Badge>
            )}
            <Button
              size="xs"
      ...
      <div style={{ flex: 1, minWidth: 0, position: "relative" }}>
        ...
        <ReactFlow
          ...
          style={{ background: "var(--mantine-color-gray-0, #f8fafc)", width: "100%", height: "100%" }}
        >
          <Background variant={BackgroundVariant.Dots} gap={24} size={1.5} color="var(--mantine-color-gray-3, #cbd5e1)" />
          <Controls position="bottom-right" showZoom={false} showFitView={false} showInteractive style={{ bottom: 16, right: 16 }} />
          <MiniMap
            position="bottom-right"
            style={{ bottom: 8, right: 8, border: "1px solid var(--mantine-color-default-border)", borderRadius: 8 }}
            nodeColor={(n) =>
              n.type === "org" ? "#2563eb"
              : n.type === "department" ? "#7c3aed"
              : n.type === "group" ? "#6b21a8"
              : "#059669"
            }
        ...
        {nodes.length > 0 && enrichedVisibleNodes.length === 0 && activeFilters.length > 0 && (
          <div style={{
            position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)",
            textAlign: "center", pointerEvents: "none",
          }}>
            <div style={{ fontSize: 13, color: "var(--mantine-color-dimmed)", marginBottom: 8 }}>
        ...
        <Toolbar
          ...
          onUndo={() => undo((ns) => setNodes(ns), (es) => setEdges(es))}
          onRedo={() => redo((ns) => setNodes(ns), (es) => setEdges(es))}
        ...
        {contextMenu && (
          ...
          >
            {getContextMenuActions(contextMenu.nodeId).map(({ label, action, danger }) => (
              <div
                key={label}
      ...
      <InspectorPanel
        ...
        onAddDivision={() => selectedNodeForDrawer && openAddModal("department", selectedNodeForDrawer.id, (selectedNodeForDrawer.data as OrgOfficeData).name)}
        onAddPerson={() => selectedNodeForDrawer && openAddModal("person", selectedNodeForDrawer.id, (selectedNodeForDrawer.data as DepartmentData).name)}
        onAddChild={() => selectedNodeForDrawer && openAddModal("department", selectedNodeForDrawer.id, (selectedNodeForDrawer.data as DepartmentData).name)}
