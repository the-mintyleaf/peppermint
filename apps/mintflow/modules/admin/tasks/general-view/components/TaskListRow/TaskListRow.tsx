...
import type { Task, TaskCategory, TaskSubtask } from "../../../kanban/module.api";
...
function ProgressBar({ pct, color, striped }: ProgressBarProps) {
  return (
    <Box className={tableClasses.bar}>
      <Box
        className={striped ? `${tableClasses.barFill} ${tableClasses.barFillStriped}` : tableClasses.barFill}
        style={{
...
function getStatusTracking(task: Task, displayStatus: DisplayStatus) {
  ...
  let striped  = false;
  if (displayStatus === "ready_for_review") barColor = "var(--mantine-color-green-5)";
  if (displayStatus === "in_review") { barColor = "var(--mantine-color-green-5)"; striped = true; }
  if (displayStatus === "rejected")  barColor = "var(--mantine-color-red-5)";

  return { pct, barColor, striped, total, left };
}

function getStatusActionLabel(task: Task, displayStatus: DisplayStatus, left: number): string {
  switch (displayStatus) {
    ...
    case "in_review": {
      ...
      const seen      = task.requestStatus?.includes("Seen") ? reviewers : (reviewers > 1 ? 1 : 0);
...
function getDeadline(task: Task) {
  ...
  if (task.startDate) {
    ...
    if (start) totalDays = Math.max(1, Math.ceil((end.getTime() - start.getTime()) / 86400000));
  }

  const remainingPct = daysLeft < 0 ? 0 : Math.min(100, (daysLeft / totalDays) * 100);

  let urgency: string;
  let urgencyColor: string;
  if (daysLeft < 0)       { urgency = "Overdue";  urgencyColor = "red"; }
  else if (daysLeft <= 7) { urgency = "Due soon"; urgencyColor = "orange"; }
  else                    { urgency = "On track"; urgencyColor = "dimmed"; }

  const barColor = daysLeft < 0
...
function getSubtaskDeadline(dueDate: string, parentStart?: string) {
  ...
  const urgency = daysLeft < 0 ? "Overdue" : daysLeft <= 3 ? "Due soon" : "On track";
  const urgencyColor = daysLeft < 0 ? "red" : daysLeft <= 3 ? "orange" : "dimmed";

  return { remainingPct, barColor, formatted, urgency, urgencyColor };
}

function getSubtaskTracking(subtask: TaskSubtask) {
  const pct = subtask.status === "completed" ? 100 : subtask.status === "in_progress" ? 50 : 0;
  const barColor = subtask.status === "completed"
    ? "var(--mantine-color-green-5)"
    : subtask.status === "in_progress"
    ? "var(--mantine-color-blue-5)"
    : "var(--mantine-color-gray-4)";
  const label = subtask.status === "completed" ? "Done" : subtask.status === "in_progress" ? "In progress" : "Pending";
  return { pct, barColor, label };
}

function assigneeNames(task: Task): string {
  const list = task.assignees ?? [{ name: task.assignee, initials: "", color: "" }];
  if (list.length === 0) return "—";
  return list.map((a) => a.name).join(", ");
}

function TrackingCell({ pct, barColor, striped, label, total, active }: {
  pct: number; barColor: string; striped?: boolean; label: string; total?: number; active?: boolean;
}) {
  return (
    <div className={tableClasses.trackingCell}>
      ...
      <div className={tableClasses.trackingMeta}>
        <span className={`${tableClasses.actionLabel} ${active ? tableClasses.actionLabelActive : tableClasses.actionLabelMuted}`}>
          {label} ›
        </span>
        {total != null && total > 0 && <span className={tableClasses.count}>{total}</span>}
      </div>
    </div>
  );
}

function DeadlineCell({ deadline, fallback }: { deadline: ReturnType<typeof getDeadline>; fallback?: string }) {
  if (!deadline) {
  ...
  return (
    <div className={tableClasses.deadlineCell}>
      ...
      <div className={tableClasses.deadlineMeta}>
        ...
        <Text component="span" size="xs" c={deadline.urgencyColor}>· {deadline.urgency}</Text>
      </div>
    </div>
  );
}
...
function SubtaskRow({
  ...
  return (
    <Box className={`${tableClasses.grid} ${tableClasses.row} ${tableClasses.subtaskRow}`}>
      <span />
      <div className={tableClasses.caseCell}>
        <span className={tableClasses.cellText} style={{ fontFamily: "monospace" }}>S-{index + 1}</span>
        <span className={tableClasses.categoryPill}>{subtask.category}</span>
      </div>
      <span className={tableClasses.titleText} style={{ fontWeight: 400, fontSize: "var(--mantine-font-size-xs)" }}>
        {subtask.title}
      </span>
      <span className={tableClasses.cellText}>{parentAssignees}</span>
      <TrackingCell pct={tracking.pct} barColor={tracking.barColor} label={tracking.label} />
  ...
  return (
    <>
      ...
      >
        ...
        <div className={tableClasses.caseCell}>
          <span className={tableClasses.cellText} style={{ fontFamily: "monospace" }}>{task.taskNumber}</span>
          <span className={tableClasses.categoryPill} title={CATEGORY_LABELS[task.category]}>{categoryLabel}</span>
        </div>
