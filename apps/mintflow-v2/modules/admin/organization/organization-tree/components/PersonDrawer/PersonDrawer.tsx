...
import {
  KpiCard, StatRow, MiniBarChart, RingChart, ActivityHeatmap, ProgressBar, makeSeededRng,
...
const ROLE_COLORS: Record<string, string> = {
  head: "violet", manager: "blue", coordinator: "teal",
  officer: "cyan", member: "gray", advisor: "orange",
  minister: "grape", secretary: "indigo", joint_secretary: "blue",
  under_secretary: "cyan", section_officer: "teal", assistant: "green",
};
...
export function PersonDrawerContent({
  ...
  const tasksDone      = data.activeTasks !== undefined ? Math.floor(data.activeTasks * 3.4) : Math.floor(rng() * 120 + 20);
  ...
  const tasksSparkData   = Array.from({ length: 8 }, () => Math.floor(rng() * 70 + 10));
  const activeSparkData  = Array.from({ length: 8 }, () => Math.floor(rng() * 30 + 5));
  const reportsSparkData = Array.from({ length: 8 }, () => Math.floor(rng() * 20 + 2));
  const perfSparkData    = Array.from({ length: 8 }, () => Math.floor(rng() * 40 + 50));
  const attendSparkData  = Array.from({ length: 8 }, () => Math.floor(rng() * 20 + 75));
      ...
      <div style={{ padding: "18px 20px 20px" }}>
        <Group gap={12} align="flex-start">
          ...
          <Stack gap={4} style={{ flex: 1, minWidth: 0 }}>
            <Text fw={700} size="md" lineClamp={1}>{data.fullName}</Text>
            <Text size="xs" c="dimmed" lineClamp={1}>{data.designation}</Text>
            <Group gap={5} mt={2}>
              <Badge size="xs" color={STATUS_COLORS[data.status]} variant="light">{data.status}</Badge>
              {data.role && (
                <Badge size="xs" color={ROLE_COLORS[data.role] ?? "gray"} variant="light">
        ...
        {(data.department || data.reportingManager || data.email || data.phone) && (
          <Stack gap={7} mt={14}>
            {data.department && (
              <Group gap={7}>
                <BuildingOfficeIcon size={13} color="var(--mantine-color-dimmed)" aria-label="Department" />
                <Text size="xs" c="dimmed" lineClamp={1}>{data.department}</Text>
              </Group>
            )}
            {data.reportingManager && (
              <Group gap={7}>
                <ArrowBendUpLeftIcon size={13} color="var(--mantine-color-dimmed)" aria-label="Reports to" />
                <Text size="xs" c="dimmed">Reports to <Text span fw={600} c="dark" size="xs">{data.reportingManager}</Text></Text>
              </Group>
            )}
            {data.email && (
              <Group gap={7}>
                <EnvelopeIcon size={13} color="var(--mantine-color-dimmed)" aria-label="Email" />
                <Text size="xs" c="dimmed" lineClamp={1}>{data.email}</Text>
              </Group>
            )}
            {data.phone && (
              <Group gap={7}>
                <PhoneIcon size={13} color="var(--mantine-color-dimmed)" aria-label="Phone" />
                <Text size="xs" c="dimmed">{data.phone}</Text>
              </Group>
            )}
          </Stack>
        )}
      ...
      <Stack gap={14} style={{ padding: "20px 16px" }}>
          ...
          <SimpleGrid cols={2} spacing={12}>
            ...
            <KpiCard title="TOTAL BELOW">
              <StatRow
                ...
                sparkData={reportsSparkData.map((v) => Math.round(v * totalBelow / Math.max(directReports, 1)))}
                sparkColor="#bfdbfe"
        ...
        <SimpleGrid cols={2} spacing={12}>
          <KpiCard title="PERFORMANCE">
            <StatRow
              ...
              valueColor={performanceScore > 85 ? "teal" : performanceScore > 70 ? "blue" : "orange"}
        ...
        <KpiCard title="MONTHLY CONTRIBUTIONS">
          ...
          <div style={{ marginTop: 16 }}>
            <MiniBarChart data={barData.map((b) => b.value)} labels={barData.map((b) => b.label)} height={96} color="#bbf7d0" activeColor="#059669" />
          </div>
        </KpiCard>
        ...
        <KpiCard title="TASK BREAKDOWN">
          <Group gap={14} align="center" wrap="nowrap">
            ...
            <Stack gap={8} style={{ flex: 1 }}>
              {[
                { label: "Completed", value: tasksDone,    color: "#059669", textColor: "teal"   as const },
                { label: "Active",    value: tasksActive,  color: "#f59e0b", textColor: "orange" as const },
                { label: "Pending",   value: tasksPending, color: "#e5e7eb", textColor: "dimmed" as const },
              ].map((item) => (
        ...
        <KpiCard title="DELIVERY METRICS">
          <Stack gap={12}>
            <ProgressBar label="Overall score"  value={performanceScore} color="#7c3aed" />
            <ProgressBar label="Attendance"     value={attendancePct}   color="#059669" />
            <ProgressBar label="Delivery rate"  value={deliveryPct}     color="#0891b2" />
          </Stack>
        </KpiCard>
        ...
        <KpiCard title="WORK OVERVIEW">
          <Group justify="space-between" mb={10}>
            ...
            <Text size="xs" fw={600} c="teal" style={{ fontSize: 10 }}>{tasksDone} activities</Text>
          </Group>
        ...
        {healthIssues.length > 0 && (
          <KpiCard style={{ background: "var(--mantine-color-orange-0)", borderColor: "var(--mantine-color-orange-2)" }}>
            <Group gap={6} mb={10}>
              ...
              <Text size="xs" fw={700} tt="uppercase" c="orange" style={{ letterSpacing: "0.06em", fontSize: 10 }}>Health</Text>
            </Group>
            <Stack gap={6}>
...
export function PersonDrawer({ opened, onClose, ...rest }: PersonDrawerProps) {
  return (
    <Drawer
      ...
      title={
        <Group gap="sm">
          <Avatar src={rest.data.avatarUrl} color="teal" radius="xl" size="md" alt={rest.data.fullName}>
            {getInitials(rest.data.fullName)}
          </Avatar>
          <Stack gap={0}>
            <Text fw={700} size="sm">{rest.data.fullName}</Text>
            <Text size="xs" c="dimmed">{rest.data.designation}</Text>
          </Stack>
        </Group>
      }
