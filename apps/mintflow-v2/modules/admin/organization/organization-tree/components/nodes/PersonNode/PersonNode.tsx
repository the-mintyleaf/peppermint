...
const ROLE_COLORS: Record<string, string> = {
  ...
  minister: "violet", secretary: "blue", joint_secretary: "indigo",
  under_secretary: "cyan", section_officer: "teal", assistant: "gray",
};

export function PersonNode({ data, selected, id }: NodeProps<PersonFlowNodeType>) {
  const [hovered, setHovered] = useState(false);
  const { openEditModal, selectNode, expandNode, collapseNode } = useOrgTreeStore();
    ...
    >
      ...
      <div className={styles.nodeBody} style={{ padding: "12px 14px" }}>
        <Group gap="sm" wrap="nowrap" align="flex-start">
          <Avatar src={data.avatarUrl} color={data.status === "inactive" ? "gray" : "teal"} radius="xl" size="md" alt={data.fullName}>
            {getInitials(data.fullName)}
          </Avatar>
          <Stack gap={2} style={{ flex: 1, minWidth: 0 }}>
            <Group gap={6} wrap="nowrap" justify="space-between">
              <Text size="sm" fw={700} lineClamp={1} c={data.status === "inactive" ? "dimmed" : undefined}>
                {data.fullName}
              </Text>
            ...
            {data.department && (
              <Text size="xs" c="dimmed" lineClamp={1} style={{ fontSize: 11 }}>{data.department}</Text>
            )}
            {data.role && (
            ...
            {directReports > 0 && (
              <Text size="xs" c="dimmed" style={{ fontSize: 11 }} mt={2}>
                ...
                {totalBelow > directReports ? ` · ${totalBelow} total below` : ""}
              </Text>
            )}
          </Stack>
        </Group>
      ...
      {hasChildren && (
        <div className={styles.nodeExpandStrip}>
          <Group gap={4} style={{ flex: 1 }} wrap="nowrap">
            {directReports > 0 && (
              <Text size="xs" c="dimmed">{directReports} report{directReports !== 1 ? "s" : ""}</Text>
            )}
            {(descendantStats?.totalPeople ?? 0) > 0 && directReports !== descendantStats?.totalPeople && (
              <Text size="xs" c="dimmed">· {descendantStats!.totalPeople} below</Text>
            )}
          </Group>
          <ActionIcon
            size="xs"
      ...
      {hovered && (
        <div className={styles.nodeActions}>
          <Tooltip label="View profile" position="top" withArrow>
            <ActionIcon size="xs" variant="light" color="teal" onClick={(e) => { e.stopPropagation(); selectNode(id); }} aria-label="View profile">
              <ArrowSquareOutIcon size={12} />
            </ActionIcon>
          </Tooltip>
          <Tooltip label="Edit" position="top" withArrow>
            <ActionIcon size="xs" variant="light" color="gray" onClick={(e) => { e.stopPropagation(); openEditModal(id); }} aria-label="Edit person">
              <PencilSimpleIcon size={12} />
            </ActionIcon>
          </Tooltip>
          <Tooltip label="Remove" position="top" withArrow>
            <ActionIcon size="xs" variant="light" color="red" onClick={(e) => e.stopPropagation()} aria-label="Remove person">
              <TrashIcon size={12} />
            </ActionIcon>
          </Tooltip>
        </div>
