...
import { Badge, Divider, Drawer, Group, SimpleGrid, Stack, Text, ThemeIcon } from "@peppermint/ui";
import { BuildingsIcon } from "@phosphor-icons/react/dist/csr/Buildings";
import { WarningIcon } from "@phosphor-icons/react/dist/csr/Warning";
import { MapPinIcon } from "@phosphor-icons/react/dist/csr/MapPin";
import { UserIcon } from "@phosphor-icons/react/dist/csr/User";
import {
  KpiCard, StatRow, StackedBar, MiniBarChart, RingChart, ActivityHeatmap, ProgressBar, makeSeededRng,
...
const STATUS_COLORS: Record<string, string> = { active: "teal", inactive: "gray", archived: "red" };
const ORG_TYPE_COLORS: Record<string, string> = {
  ministry: "violet", office: "blue", department: "cyan",
  organization: "teal", branch: "orange", district: "red",
};
...
export function OrgDrawerContent({
  ...
  const healthScore    = Math.max(20, 100 - healthIssues.length * 18 - Math.floor(rng() * 15));
  ...
  const peopleSparkData = Array.from({ length: 8 }, () => Math.floor(rng() * 60 + 20));
  const unitsSparkData  = Array.from({ length: 8 }, () => Math.floor(rng() * 30 + 5));
  const tasksSparkData  = Array.from({ length: 8 }, () => Math.floor(rng() * 80 + 20));
  const healthSparkData = Array.from({ length: 8 }, () => Math.floor(rng() * 40 + 50));
  ...
  return (
    <Stack gap={0}>
      ...
      <div style={{ padding: "18px 20px 20px" }}>
        <Group gap={6} mb={10}>
          <Badge size="sm" color={ORG_TYPE_COLORS[data.orgType] ?? "blue"} variant="light">{data.orgType}</Badge>
          <Badge size="sm" color={STATUS_COLORS[data.status]} variant="dot">{data.status}</Badge>
        </Group>
        ...
        <Stack gap={7}>
          {data.location && (
            <Group gap={7}>
              <MapPinIcon size={13} color="var(--mantine-color-dimmed)" aria-label="Location" />
          ...
          {headPersonName && (
            <Group gap={7}>
              ...
              <Text size="xs" c="dimmed">Head: <Text span fw={600} c="dark" size="xs">{headPersonName}</Text></Text>
            </Group>
          )}
        </Stack>
      </div>
      ...
      <Stack gap={14} style={{ padding: "20px 16px" }}>
        ...
        <SimpleGrid cols={2} spacing={12}>
          ...
          <KpiCard title="HEALTH">
            <StatRow
              title="Score"
              value={`${healthScore}%`}
              valueColor={healthScore > 70 ? "teal" : healthScore > 40 ? "orange" : "red"}
              delta={healthScore > 70 ? "Good" : healthScore > 40 ? "Fair" : "Poor"}
        ...
        <KpiCard title="STRUCTURE BREAKDOWN">
          <Group gap={12} align="center" wrap="nowrap" mb={14}>
            ...
            <Stack gap={8} style={{ flex: 1 }}>
              <Group gap={6} justify="space-between">
                <Group gap={6}><div style={{ width: 8, height: 8, borderRadius: 2, background: "#7c3aed", flexShrink: 0 }} /><Text size="xs" c="dimmed">People</Text></Group>
                <Text size="xs" fw={700} c="violet">{totalPeople.toLocaleString()}</Text>
              </Group>
              <Group gap={6} justify="space-between">
                <Group gap={6}><div style={{ width: 8, height: 8, borderRadius: 2, background: "#06b6d4", flexShrink: 0 }} /><Text size="xs" c="dimmed">Units</Text></Group>
                <Text size="xs" fw={700} c="cyan">{totalDepts.toLocaleString()}</Text>
              </Group>
              {(hiddenLevels ?? 0) > 0 && (
                <Group gap={6} justify="space-between">
                  <Group gap={6}><div style={{ width: 8, height: 8, borderRadius: 2, background: "var(--mantine-color-gray-4)", flexShrink: 0 }} /><Text size="xs" c="dimmed">Hidden</Text></Group>
                  <Text size="xs" fw={700} c="dimmed">{hiddenLevels}</Text>
                </Group>
              )}
            </Stack>
          </Group>
          <StackedBar
            segments={[
              { label: "People", value: totalPeople, color: "#7c3aed", delta: `+${peopleDelta}`, deltaPositive: true },
              { label: "Units",  value: totalDepts,  color: "#06b6d4", delta: `+${unitsDelta}`,  deltaPositive: true },
            ]}
          />
        </KpiCard>
        ...
        <KpiCard title="PERFORMANCE METRICS">
          <Stack gap={12}>
            <ProgressBar label="Task completion" value={tasksDone} max={tasksDone + tasksActive} color="#7c3aed" />
            <ProgressBar label="Staff capacity"  value={staffCapacity} color="#06b6d4" />
            <ProgressBar label="Budget utilization" value={budgetUtil} color="#f59e0b" />
          </Stack>
        </KpiCard>
        ...
        <KpiCard title="WORK OVERVIEW">
          <Group justify="space-between" mb={10}>
            ...
            <Text size="xs" fw={600} c="violet" style={{ fontSize: 10 }}>{tasksDone} total</Text>
          </Group>
    ...
    <Drawer
      ...
      title={
        <Group gap="xs">
          ...
          <Text fw={600} size="sm">Organization</Text>
        </Group>
      }
      position="right"
      size="sm"
