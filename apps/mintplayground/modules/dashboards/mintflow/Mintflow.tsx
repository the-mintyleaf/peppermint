...
import type { DashboardVariant } from "./Mintflow.hooks";
...
export function ModuleMintflow() {
  ...
  return (
    <>
      <ModuleHeader
        breadcrumbItems={[{ label: "Mintflow", href: "/dashboards/mintflow" }]}
        right={
          <Group gap="xs" mr="sm">
            <Button
              size="xs"
        ...
        <ScrollArea h="100%">
          {isLoading ? (
            ...
            >
              ...
              <Stack gap={0} style={{ flex: "1 1 560px", minWidth: 0 }}>
                ...
                <Box style={{ padding: "18px 20px 22px" }}>
                  <TaskFlowBoard
                    ...
                    onOpenTasks={() => router.push("/mintflow/tasks")}
                  />
                </Box>
              </Stack>
              ...
              {/* Rail — awaiting you (live) → attention → work files → this
                  week + momentum. "Awaiting you" leads: it's the only real
                  data and answers "what's on me right now?" */}
              <Stack
                gap={0}
                style={{
                  flex: "1 1 320px",
