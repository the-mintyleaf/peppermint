    ...
    <Stack gap="md">
      <Paper p="lg" radius="md" withBorder>
        <Group justify="space-between">
          <Stack gap={4}>
            <Title order={3}>Billing & Plan</Title>
            <Text c="dimmed" size="sm">Manage your subscription, usage, and invoices</Text>
          </Stack>
        </Group>
      </Paper>
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
        <Paper withBorder radius="md" p="md">
          <Stack gap="sm">
            ...
            <Text size="xl" fw={700}>${currentPlan.price}<Text component="span" size="sm" c="dimmed" fw={400}>/{currentPlan.period}</Text></Text>
            <Text size="xs" c="dimmed">Next billing: {nextBillingDate.toLocaleDateString()}</Text>
            <Text size="xs" c="dimmed">Payment: {paymentMethod.brand} ending in {paymentMethod.last4}</Text>
          ...
          {availablePlans.map((plan) => (
            ...
            >
              <Stack gap="sm">
                ...
                <Text fw={700}>${plan.price}<Text component="span" size="xs" c="dimmed" fw={400}>/mo</Text></Text>
                <Stack gap="xs">
                  {plan.features.map((f) => (
                ...
                {plan.id !== currentPlan.id && (
                  <Button size="xs" variant={plan.price > currentPlan.price ? "filled" : "light"} fullWidth>
