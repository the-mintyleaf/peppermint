...
const MOCK_NODES: AutomationNode[] = [
  ...
  },
  {
    ...
    meta: { template_id: "tmpl_1", template_name: "Instagram Square v2" },
  },
  {
    id: "n5",
    type: "branch",
...
let mockAutomations: Automation[] = [
  {
    ...
    description:
      "Generates and publishes one Instagram post every Monday at 9am",
    status: "scheduled",
  ...
  },
  {
    ...
    description:
      "Publishes a long-form article to LinkedIn on the first Monday of each month",
    status: "paused",
...
const mockRunsMap: Record<string, Run[]> = {
  auto_1: Array.from({ length: 10 }, (_, i) => ({
    ...
    endedAt: new Date(
      Date.now() - (i + 1) * 7 * 86_400_000 + 120_000,
    ).toISOString(),
    ...
    steps: MOCK_NODES.map((n) => ({
      ...
      nodeLabel: n.label,
      status:
        i === 2 && ["n5", "n6", "n7"].includes(n.id) ? "error" : "success",
...
export async function fetchAutomations(filter?: {
  ...
  return {
    data: results,
    meta: { total: results.length, page: 1, pageSize: 50 },
  };
}
