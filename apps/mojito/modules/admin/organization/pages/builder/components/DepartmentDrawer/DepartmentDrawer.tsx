      ...
      title={
        <Group gap="sm">
          ...
          <Stack gap={0}>
            ...
            <Text size="xs" c="dimmed" tt="capitalize">{data.deptType}</Text>
          </Stack>
        </Group>
      }
      styles={{
        ...
        <Stack gap="md">
            ...
            <Stack gap="xs">
              {data.parentName && (
                <Group gap="xs" justify="space-between">
                  ...
                  <Badge variant="outline" size="xs" color="gray">{data.parentName}</Badge>
                </Group>
              )}
              ...
              {data.head && (
                <Group gap="xs" justify="space-between">
                  ...
                  <Text size="xs" fw={600}>{data.head}</Text>
                </Group>
              )}
            </Stack>
          </div>
          ...
          <Group gap="xs" grow>
            <div className={styles.statBadge}>
              <Text size="lg" fw={700} c="violet">{data.peopleCount ?? 0}</Text>
              <Text size="xs" c="dimmed">People</Text>
            </div>
            <div className={styles.statBadge}>
              <Text size="lg" fw={700} c="orange">{data.activeTasks ?? 0}</Text>
              <Text size="xs" c="dimmed">Active</Text>
            </div>
            <div className={styles.statBadge}>
              <Text size="lg" fw={700} c="yellow">{data.pendingTasks ?? 0}</Text>
              <Text size="xs" c="dimmed">Pending</Text>
            </div>
            <div className={styles.statBadge}>
              <Text size="lg" fw={700} c="teal">{data.completedTasks ?? 0}</Text>
          ...
          {data.assignedPeople && data.assignedPeople.length > 0 && (
            <div className={styles.drawerSection}>
              <Group gap="xs" justify="space-between" mb={8}>
                ...
                <ActionIcon size="xs" variant="light" color="teal" onClick={onAddPerson} aria-label="Add person">
              ...
              <Stack gap="xs">
                {data.assignedPeople.map((p) => (
                  <Group key={p.id} gap="xs">
                    <Avatar size="sm" color="teal" radius="xl">{p.name[0]}</Avatar>
                    <Stack gap={0}>
                      <Text size="xs" fw={600}>{p.name}</Text>
                      {p.designation && <Text size="xs" c="dimmed">{p.designation}</Text>}
                    </Stack>
                  </Group>
                ))}
              </Stack>
            ...
            <Group gap="md" justify="space-between">
              {data.createdAt && (
                <Stack gap={0}>
                  ...
                  <Text size="xs" fw={500}>{data.createdAt}</Text>
                </Stack>
              )}
              {data.updatedAt && (
                <Stack gap={0}>
                  <Text size="xs" c="dimmed">Last updated</Text>
                  <Text size="xs" fw={500}>{data.updatedAt}</Text>
                </Stack>
              )}
            </Group>
          )}
          ...
          <Stack gap="xs">
            ...
            <Group gap="xs" wrap="wrap">
              ...
              <Button
                ...
                variant="light"
                color="blue"
                leftSection={<PlusIcon size={12} aria-label="Add child department" />}
                onClick={onAddChild}
              >
                Add Child Dept
              </Button>
          ...
          <Button
            ...
            variant="light"
            color="blue"
            rightSection={<ArrowSquareOutIcon size={14} aria-label="Open full details" />}
          >
            View Full Details
          </Button>
        </Stack>
