      ...
      {isLoading ? (
        ...
        <>
          <SimpleGrid cols={{ base: 2, md: 4 }} spacing="md">
            <KPICard
              label="Impressions"
              value={data.kpis.impressions.toLocaleString()}
              delta={data.kpis.impressionsDelta}
            />
            <KPICard
              label="Reach"
              value={data.kpis.reach.toLocaleString()}
              delta={data.kpis.reachDelta}
            />
            <KPICard
            ...
            <KPICard
              ...
              value={`${data.kpis.followerGrowth}%`}
            />
          </SimpleGrid>
          ...
          <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
            ...
            <Paper withBorder radius="md" p="md">
              ...
              <DonutChart
                h={200}
                data={data.platformBreakdown.map((p) => ({
                  name: p.platform,
                  value: p.pct,
                  color: "blue",
                }))}
              ...
              <Table.Tbody>
                {data.topPosts.map((p) => (
                  <Table.Tr key={p.id}>
                    ...
                    <Table.Td>
                      <Text size="sm">{p.impressions.toLocaleString()}</Text>
                    </Table.Td>
                    <Table.Td>
                      <Text size="sm">{p.engagement.toLocaleString()}</Text>
                    </Table.Td>
                  </Table.Tr>
                ))}
              </Table.Tbody>
