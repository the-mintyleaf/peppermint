  ...
  return (
    <>
      <Card
        ...
        onClick={() =>
          router.push(`/admin/automation/templates/${template.id}/edit`)
        }
      >
        <Card.Section>
          <Image
        ...
        >
          ...
          <Stack gap={0} style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text size="10px" c="dimmed" truncate>
              {formatEditedAgo(template.updatedAt)} ·{" "}
          ...
          <Menu withArrow position="bottom-end" width={160}>
            ...
            <Menu.Dropdown>
              ...
              <Menu.Item
                ...
                onClick={() =>
                  router.push(
                    `/admin/automation/templates/${template.id}/preview`,
                  )
                }
              >
                Preview
      ...
      >
        <Stack gap="sm">
          {activeAutomations.length > 0 ? (
            ...
            >
              This template is used by {activeAutomations.length} active
              automation
              {activeAutomations.length > 1 ? "s" : ""}:{" "}
              {activeAutomations.map((a) => a.name).join(", ")}. Pause or delete
              those automations before deleting this template.
            </Alert>
          ) : (
            <>
              <Text size="sm">
                ...
                {referencingAutomations &&
                  referencingAutomations.length > 0 && (
                    <>
                      {" "}
                      This template is referenced by{" "}
                      {referencingAutomations.map((a) => a.name).join(", ")} but
                      those automations are not currently active.
                    </>
                  )}
              </Text>
