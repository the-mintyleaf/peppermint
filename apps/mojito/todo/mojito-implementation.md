...
- [ ] `components/HashtagManager/` — insert hashtag, saved groups panel _(deferred)_
- [ ] `components/LinkInsert/` — URL input + UTM builder _(deferred)_
- [x] `components/MediaPanel/` — upload (local object URL), reorder, preview grid
- [ ] `components/FormatSwitcher/` — format selector per variant _(deferred)_
- [ ] `components/FirstCommentInput/` — collapsible first comment field _(deferred)_
...
- [ ] Add trigger config panel (schedule/webhook/manual, cron, next-run preview) _(deferred)_
