...
| 1.7.0   | 2026-07-19 | AI (Claude) | Phase 7 — `Lead` intake model + `LeadCodeCounter`, `EducationLevel`/`PaymentStatus` enums, new `Applicant.payment_status` field, lead→applicant conversion mapping. Direct applicant creation is now admin-only. |
...
## 15. Lead intake (Phase 7)

**Purpose:** `Lead` is a staff-owned enquiry record captured at first contact — the funnel step _before_ an `Applicant` exists (concept §6.1, "minimal data acceptable"). Staff own the whole lead lifecycle; an admin/superadmin **converts** it into an `Applicant`, migrating a curated field subset and leaving the enquiry snapshot on the lead. Direct applicant creation became admin-only in this phase, so staff funnel exclusively through leads.

**Table:** `applicant_lead`
**`education_level` choices:** `diploma`, `bachelor`, `post_graduate`, `masters`, `others`
**`payment_status` choices:** `prepaid`, `postpaid` (blank = unset)
**`lead_source` choices:** reuses `LeadSource` (`walk_in`, `referral`, `online`, `phone`, `social_media`, `event`, `agent`, `other`)

| Field                                               | Type               | Required | Nullable | Generated | Description                                                                                          |
| --------------------------------------------------- | ------------------ | -------- | -------- | --------- | ---------------------------------------------------------------------------------------------------- |
| `id`                                                | UUID               | —        | No       | Yes       | Public identifier.                                                                                   |
| `lead_code`                                         | char(20)           | —        | No       | Yes       | Immutable `LEAD-YYYY-NNNNNN` (ASCII, §39.7).                                                         |
| `first_name`                                        | char(150)          | Yes      | No       | No        | Only required capture field.                                                                         |
| `middle_name`/`last_name`                           | char(150)          | No       | No       | No        | Blank-allowed name parts.                                                                            |
| `full_name`                                         | char(300)          | No       | No       | No        | Explicit or composed from parts.                                                                     |
| `name_native`                                       | char(300)          | No       | No       | No        | Devanagari name.                                                                                     |
| `full_name_romanized`                               | char(300)          | No       | No       | Yes       | ASCII search projection (§39.3); never user-entered.                                                 |
| `email`                                             | email              | No       | No       | No        | Migrated → `Applicant.primary_email`.                                                                |
| `contact_number`                                    | char(32)           | No       | No       | No        | Migrated → `Applicant.primary_phone`.                                                                |
| `address`                                           | text               | No       | No       | No        | Migrated → `ApplicantAddress.address_text` (current, primary).                                       |
| `passport_number`                                   | char(100)          | No       | No       | No        | Sensitive; never logged. Migrated → `ApplicantIdentityDocument` (passport).                          |
| `guardian_name`                                     | char(200)          | No       | No       | No        | Enquiry-only.                                                                                        |
| `guardian_contact`                                  | char(32)           | No       | No       | No        | Enquiry-only; never logged.                                                                          |
| `date_of_birth`                                     | date               | No       | Yes      | No        | Migrated → `Applicant.date_of_birth`.                                                                |
| `education_qualification`                           | JSON (list)        | No       | No       | No        | Enquiry-only snapshot; ≤50 flat objects. **Not** migrated (distinct from `ApplicantEducation`).      |
| `work_experience`                                   | JSON (list)        | No       | No       | No        | Enquiry-only snapshot; ≤50 flat objects. **Not** migrated (distinct from `ApplicantWorkExperience`). |
| `education_level`                                   | char(16)           | No       | No       | No        | Enquiry-only enum (see choices).                                                                     |
| `has_applied_visa_before`                           | bool               | No       | Yes      | No        | Enquiry-only; null = unknown.                                                                        |
| `payment_status`                                    | char(16)           | No       | No       | No        | Migrated → `Applicant.payment_status`.                                                               |
| `lead_source`/`lead_source_detail`                  | char               | No       | No       | No        | Migrated → the same `Applicant` fields.                                                              |
| `notes`                                             | text               | No       | No       | No        | Free-text; enquiry-only.                                                                             |
| `record_version`                                    | int ≥1             | —        | No       | Yes       | Optimistic-concurrency counter (§5.4).                                                               |
| `converted_applicant`                               | FK `Applicant`     | No       | Yes      | Yes       | Set on conversion; PROTECT.                                                                          |
| `converted_at`/`converted_by`                       | datetime / FK user | No       | Yes      | Yes       | Conversion stamp.                                                                                    |
| `created_by`/`updated_by`/`created_at`/`updated_at` | —                  | —        | —        | Yes       | Actor stamps (`ActorStampedModel`).                                                                  |

**Validation Rules:** `first_name` required, non-blank; every other field optional (minimal-data rule). `date_of_birth` not in the future. `education_qualification`/`work_experience` each a list of ≤50 objects. All user-entered text NFC-normalized (§39.2); `email` lowercased. `is_converted` is a derived property (`converted_applicant_id is not None`).

**Indexes:** single-field `db_index` on `full_name`, `full_name_romanized`, `lead_source`, `payment_status`, `converted_at`, `created_at` — support lead list filtering/search (§18.3) and the funnel/payment filters. `CheckConstraint` `record_version >= 1`.

**Soft Delete:** N/A — leads are not soft-deleted; a converted lead is frozen (immutable via the service) and retained as history behind its applicant.

**`LeadCodeCounter`** (`applicant_leadcodecounter`) — per-year monotonic counter, `select_for_update`-locked inside the create transaction so concurrent creates never collide on `LEAD-YYYY-NNNNNN`. Not exposed via the API.

**Conversion mapping (lead → tuned models).** Migrated: `first_name`/`middle_name`/`last_name`/`full_name`/`name_native` → Applicant names; `email` → `primary_email`; `contact_number` → `primary_phone`; `date_of_birth`; `lead_source`/`lead_source_detail`; `payment_status`. Child records created: `address` → `ApplicantAddress` (current, primary); `passport_number` → `ApplicantIdentityDocument` (passport, fingerprinted); the conversion-time `target_country` (not stored on the lead) → `ApplicantInterestProfile.preferred_countries`. **Stay enquiry-only on the lead:** guardian name/contact, `education_qualification`, `work_experience`, `education_level`, `has_applied_visa_before`. The created applicant starts at `interested`.

**`Applicant` (extended).** New `payment_status` field (`char(16)`, choices `prepaid`/`postpaid`, blank = unset, `db_index=True`) — pre-pay vs post-pay, populated from the originating lead on conversion; admin-editable thereafter, never staff-editable.

**Cross-App Dependencies (Phase 7).** Reuses `applicant.services.applicant.create_applicant`, `profile.create_address`, `children.create_child` (identity-document spec), and `interest_profile.create_interest_profile` for conversion — no new cross-app coupling.
