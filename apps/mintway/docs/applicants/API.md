...
| 1.7.0   | 2026-07-19 | AI (Claude) | Phase 7 — lead intake (staff CRUD) + admin lead→applicant conversion; **direct applicant create is now admin-only** (staff → 403).                                         |
...
**Access level:** **admin/superadmin only (Phase 7).** Staff → `403 APPLICANT_FIELD_FORBIDDEN` — staff capture enquiries via the lead intake (§15), which an admin then converts. `ApplicantWriteAdminSerializer` (adds DOB/gender/religion/summary/follow-up/`payment_status`; `lifecycle_stage` not writable, forced `interested`/`active`).
**Response:** admin detail projection (`201`). `meta.possible_duplicate` + masked `matches[]` when an exact email/phone match exists (non-blocking, §11.2).
**Validation:** `first_name` required; text NFC-normalized.
...
## 15. Lead intake (Phase 7) — staff-owned; conversion admin-only

Staff-owned enquiry funnel that precedes an `Applicant`. All fields optional but `first_name` (concept "minimal data acceptable"). See `DATA_CONTRACT.md §15` for the field table and the conversion mapping.

### 15.1 List / search — `GET /api/v1/applicants/leads/`

**Policy key:** `applicant.lead.list` (risk: low). Query params: `search` (name/romanized/native/code/email/contact), `lead_source`, `payment_status`, `education_level`, `converted` (bool), `ordering` over `{full_name, created_at, updated_at, lead_code}`.
**Response:** paginated `LeadReadSerializer`.
**Query access pattern:** `selectors.search_leads` — `__icontains` OR across the name/code/email/contact projections; single-field indexes back the enum/funnel filters.

### 15.2 Create — `POST /api/v1/applicants/leads/`

**Policy key:** `applicant.lead.create` (risk: low). Staff and above. `LeadWriteSerializer`.
**Validation:** `first_name` required, non-blank; `date_of_birth` not future; `education_qualification`/`work_experience` each a list of ≤50 objects; text NFC-normalized, `email` lowercased.
**Business rules:** generates `lead_code` (`LEAD-YYYY-NNNNNN`); `record_version=1`; emits `lead.created`. **Response:** `LeadReadSerializer` (`201`).

### 15.3 Read — `GET /api/v1/applicants/leads/{lead_id}/`

**Policy key:** `applicant.lead.read` (risk: low). `LeadReadSerializer` (includes `date_of_birth_bs`, `is_converted`, `converted_applicant_code`). `APPLICANT_LEAD_NOT_FOUND` (404 — also malformed UUID, non-disclosing).

### 15.4 Update — `PATCH /api/v1/applicants/leads/{lead_id}/`

**Policy key:** `applicant.lead.update` (risk: low). Partial `LeadWriteSerializer`; optional `record_version` (checked when supplied).
**Errors:** `APPLICANT_LEAD_VERSION_CONFLICT` (409), `APPLICANT_LEAD_ALREADY_CONVERTED` (409 — a converted lead is frozen), `APPLICANT_FIELD_FORBIDDEN` (403 — non-whitelist field).
**Business rules:** bumps `record_version`; recomputes the romanized projection; emits `lead.updated`.

### 15.5 Convert — `POST /api/v1/applicants/leads/{lead_id}/convert/`

**Policy key:** `applicant.lead.convert` (risk: high). **Admin/superadmin only** (staff → 403). Body `{ target_country (required), record_version? }`.
**Behaviour:** creates an `interested` `Applicant` (migrating name/contact/email/DOB/`payment_status`), an `ApplicantAddress` (from `address`), an `ApplicantIdentityDocument` (from `passport_number`), and an `ApplicantInterestProfile` seeded with `target_country`; stamps `converted_applicant`/`converted_at`/`converted_by` on the lead and freezes it. Enquiry-only fields (guardian, education/work JSON, education level, visa-before) stay on the lead. Emits `lead.converted`.
**Response:** `{ lead, applicant }` (`201`) + `meta.possible_duplicate`/`matches[]` (non-blocking).
**Errors:** `APPLICANT_LEAD_CONVERT_COUNTRY_REQUIRED` (400), `APPLICANT_LEAD_ALREADY_CONVERTED` (409), `APPLICANT_LEAD_VERSION_CONFLICT` (409), `APPLICANT_LEAD_NOT_FOUND` (404).
...
| `APPLICANT_LEAD_NOT_FOUND`                          | 404  | Unknown lead (also malformed UUID, non-disclosing)              |
| `APPLICANT_LEAD_VERSION_CONFLICT`                   | 409  | Stale lead `record_version`                                     |
| `APPLICANT_LEAD_ALREADY_CONVERTED`                  | 409  | Update/convert on an already-converted lead                     |
| `APPLICANT_LEAD_CONVERT_COUNTRY_REQUIRED`           | 400  | Convert without a `target_country`                              |
