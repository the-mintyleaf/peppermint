...
| Signatures       | `modules/admin/signatures`                    | `/admin/signatures`                   | admin-only signatory CRUD on `ModalTableShell` + `FormWrapper` (`pages/list/`, `form/`); `SignatureRowActionsMenu` (edit + deactivate); shares the engine signature API via `signatures.api.ts`, invalidates the editor's `documentQueryKeys.signatures()` on write |
