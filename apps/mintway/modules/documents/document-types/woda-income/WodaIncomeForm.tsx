"use client";

import { WodaForm } from "../../components/WodaForm";
import type { DocumentFormProps } from "../../documents.types";
import type { WodaFormSchema } from "../../utils/wodaFormSchema";
import {
  applicantSection,
  documentSection,
  earningGuardianField,
  parentsSection,
  spokespersonSection,
} from "../../utils/wodaCommonSections";

const schema: WodaFormSchema = {
  sections: [
    documentSection(),
    applicantSection({ address: true }),
    parentsSection(),
    {
      title: "Fiscal years",
      description: "The three Nepali fiscal years the income covers.",
      fields: [
        earningGuardianField(),
        {
          name: "fiscal_1_bs",
          label: "Year 1 (B.S.)",
          placeholder: "2079/080",
          half: true,
        },
        {
          name: "fiscal_1_ad",
          label: "Year 1 (A.D.)",
          placeholder: "2022/023",
          half: true,
        },
        {
          name: "fiscal_2_bs",
          label: "Year 2 (B.S.)",
          placeholder: "2080/081",
          half: true,
        },
        {
          name: "fiscal_2_ad",
          label: "Year 2 (A.D.)",
          placeholder: "2023/024",
          half: true,
        },
        {
          name: "fiscal_3_bs",
          label: "Year 3 (B.S.)",
          placeholder: "2081/082",
          half: true,
        },
        {
          name: "fiscal_3_ad",
          label: "Year 3 (A.D.)",
          placeholder: "2024/025",
          half: true,
        },
      ],
    },
    {
      title: "Income & exchange rate",
      fields: [
        {
          name: "occupations",
          label: "Income sources",
          control: "occupations",
          description:
            "One row per source; enter the amount earned in each of the three years.",
          occupationColumns: [
            { key: "name", label: "Source", type: "text", width: 150 },
            { key: "income1", label: "Year 1 (Rs.)", type: "number" },
            { key: "income2", label: "Year 2 (Rs.)", type: "number" },
            { key: "income3", label: "Year 3 (Rs.)", type: "number" },
          ],
        },
        {
          name: "usd_rate",
          label: "Exchange rate (NPR per 1 US$)",
          control: "number",
          decimalScale: 2,
          description:
            "Nepal Rastra Bank rate used to convert the total to USD.",
          placeholder: "133.50",
          half: true,
        },
        {
          name: "rate_date",
          label: "Rate date (A.D.)",
          control: "date-ad",
          description: "Date the NRB rate was quoted.",
          half: true,
        },
      ],
    },
    spokespersonSection(),
  ],
};

export function WodaIncomeForm(props: DocumentFormProps) {
  return <WodaForm schema={schema} {...props} />;
}
