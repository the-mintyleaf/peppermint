...
import { Anchor, Box, Group, Stack, Text, Title } from "@peppermint/ui";

import classes from "../../../_shared/modernLines.module.css";
...
/** Step counter shown in the bottom status rail. */
const STEP_META: Record<PasswordChangePhase, string> = {
  form: "01/02 — New password",
  success: "02/02 — Updated",
  redirecting: "02/02 — Updated",
};

/** Section marker above the heading, mirroring the sign-in variant's. */
const SECTION_META: Record<PasswordChangePhase, string> = {
  form: "Credentials",
  success: "Confirmed",
 ...
 */
export function PasswordChangeLayoutModernLines({
  controller,
  page,
}: PasswordChangeLayoutProps) {
  const {
  ...
  const pageHeading = headingForPhase(heading, phase);
