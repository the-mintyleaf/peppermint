...
export const DataTableDataContext =
  createContext<DataTableDataContextValue<any> | null>(null);
DataTableDataContext.displayName = "DataTableDataContext";

export const DataTableStoreContext =
  createContext<DataTableStoreContextValue | null>(null);
DataTableStoreContext.displayName = "DataTableStoreContext";

export function useDataTableDataContext<
  T = unknown,
>(): DataTableDataContextValue<T> {
  const ctx = useContext(DataTableDataContext);
  if (!ctx)
    throw new Error("useTableData must be used inside <DataTableWrapper>");
...
export function useDataTableStoreContext(): DataTableStoreContextValue {
  ...
  if (!ctx)
    throw new Error("useTableStore must be used inside <DataTableWrapper>");
  return ctx;
}
