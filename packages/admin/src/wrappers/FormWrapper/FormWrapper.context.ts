...
export const FormInstanceContext = createContext<FormInstanceContextValue<any> | null>(null);
...
export function useFormInstanceContext<T extends FormValues>(): FormInstanceContextValue<T> {
  const ctx = useContext(FormInstanceContext);
  if (!ctx) throw new Error('useFormInstance must be used inside <FormWrapper>');
...
export function useFormControlsContext(): FormControlsContextValue {
  ...
  if (!ctx) throw new Error('useFormControls must be used inside <FormWrapper>');
  return ctx;
}
