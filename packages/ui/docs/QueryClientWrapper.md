...
This mounts `QueryClientWrapper` around `MantineProvider` and `ModalsProvider`. `configureApiClient()` is called separately in `app/layout.tsx` — `AppWrapper` does not import `@zetsel/api-client`.
